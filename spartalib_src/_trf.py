...
def trf(fun, jac, x0, f0, J0, lb, ub, ftol, xtol, gtol, max_nfev, x_scale,
        loss_function, tr_solver, tr_options, verbose, callback):
...
def trf_bounds(fun, jac, x0, f0, J0, lb, ub, ftol, xtol, gtol, max_nfev,
               x_scale, loss_function, tr_solver, tr_options, verbose, callback):
    x = x0.copy()
    ...
    while True:
        ...
        if callback is not None:
            intermediate_result = OptimizeResult(x=x, cost=cost, fun=f_true, jac=J, grad=g, optimality=g_norm, nfev=nfev, njev=njev)
